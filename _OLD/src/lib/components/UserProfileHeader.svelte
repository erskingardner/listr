...
<div class="profileWrapper relative w-full mb-14">
    ...
    bg-gradient-to-b from-transparent to-zinc-50 dark:to-zinc-950"
        />
        <div class="relative profileMetadata flex flex-col gap-2 pt-10 ml-10 max-w-lg">
            <Avatar src={user.image} class="w-36 h-36 border-zinc-100 object-cover" border />
            <h1 class="text-2xl font-bold">
                {user.displayableName()}
            </h1>
            {#if user.nip05}
                <p class="flex flex-row gap-1 text-lg font-medium items-center">
                    {user.truncatedNip05()}
                    <VerifiedCheckIcon klass="w-5 h-5" />
                </p>
            {/if}

            <p class="font-medium break-words">{user.about}</p>
        </div>
    {/if}
</div>
